...
def main():
    woodpeckerlog.info('Starting the Woodpecker Network Scanner.')
    ...
    if Args.target == local_ip:
        woodpeckerlog.info('Scanning the local ip address: ')
        woodpeckerlog.info(local_ip)
    else:
        woodpeckerlog.info('Scanning ip address(es): ')
        woodpeckerlog.info(Args.ips)
    results = []
